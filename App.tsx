...
const App: React.FC = () => {
    ...
    try {
      ...
      if (cloudAttendance && cloudAttendance.length > 0) {
        setAttendance(cloudAttendance.map(a => ({ 
          id: a.id, 
          userId: a.user_id, 
          userName: currentUsers.find(u => u.id === a.user_id)?.name || 'Unknown',
          date: a.date,
          checkIn: a.check_in,
      ...
      if (cloudTimetable) {
        setTimetable(cloudTimetable.map(t => ({
          ...
          room: t.room,
          date: t.date,
          isSubstitution: t.is_substitution,
          blockId: t.block_id,
      ...
      if (cloudSubs) {
        setSubstitutions(cloudSubs.map(s => ({
          ...
          section: s.section as SectionType,
  ...
  useEffect(() => {
    if (IS_CLOUD_ENABLED) syncFromCloud();
  }, [syncFromCloud]);
  ...
  const renderActiveTab = () => {
    switch (activeTab) {
      case 'dashboard':
        return <Dashboard user={currentUser} attendance={attendance} setAttendance={setAttendance} substitutions={substitutions} currentOTP={attendanceOTP} setOTP={setAttendanceOTP} notifications={notifications} setNotifications={setNotifications} showToast={showToast} />;
      case 'history':
        return <AttendanceView user={currentUser} attendance={attendance} setAttendance={setAttendance} users={users} showToast={showToast} />;
      case 'users':
      ...
      case 'reports':
        return <ReportingView user={currentUser} users={users} attendance={attendance} config={schoolConfig} substitutions={substitutions} />;
      case 'profile':
        return <ProfileView user={currentUser} setUsers={setUsers} setCurrentUser={setCurrentUser} />;
      default:
        return <Dashboard user={currentUser} attendance={attendance} setAttendance={setAttendance} currentOTP={attendanceOTP} setOTP={setAttendanceOTP} notifications={notifications} setNotifications={setNotifications} showToast={showToast} />;
    }
  };
