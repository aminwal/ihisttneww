...
const OtpManagementView: React.FC<OtpManagementViewProps> = ({ otp, setOtp, showToast }) => {
  const generateNewKey = () => {
    const newKey = Math.floor(100000 + Math.random() * 900000).toString();
    setOtp(newKey);
    localStorage.setItem('ihis_attendance_otp', newKey);
    showToast("Institutional Access Key Rotated", "success");
  };
    ...
    <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in duration-700 pb-20">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[3rem] p-10 md:p-16 shadow-2xl border border-slate-100 dark:border-slate-800 text-center space-y-10">
        <div className="space-y-4">
          <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em]">Current Institutional Key</p>
        ...
        <div className="space-y-6">
          <p className="text-xs text-slate-500 dark:text-slate-400 font-medium leading-relaxed max-w-md mx-auto">
            ...
            <span className="text-amber-600 dark:text-amber-400 font-black block mt-2 italic">Standard protocol: Rotate key every 24 hours.</span>
          </p>
          
          <button 
            onClick={generateNewKey}
            className="bg-[#001f3f] text-[#d4af37] px-12 py-6 rounded-[2rem] font-black text-xs uppercase tracking-[0.4em] shadow-2xl hover:bg-slate-950 transition-all active:scale-95 border-2 border-white/10"
          >
            Rotate Security Key
          </button>
        </div>
      ...
      <div className="bg-amber-50 dark:bg-amber-900/10 p-8 rounded-[2.5rem] border-2 border-amber-100 dark:border-amber-900/40 flex items-start gap-5">
        ...
        <div className="space-y-1">
          <p className="text-[10px] font-black text-amber-700 dark:text-amber-400 uppercase tracking-widest leading-none">Security Advisory</p>
          <p className="text-[11px] text-amber-600/80 dark:text-amber-500/60 font-medium leading-relaxed">Manual overrides bypass all geolocation filters. Only distribute the key to verified personnel on campus. Each rotation invalidates the previous key immediately across all terminals.</p>
        </div>
      </div>
    </div>
  );
