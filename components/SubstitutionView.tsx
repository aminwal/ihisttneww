...
const SubstitutionView: React.FC<SubstitutionViewProps> = ({ user, users, attendance, timetable, setTimetable, substitutions, setSubstitutions, assignments, config, setNotifications }) => {
  ...
  const handleScanForAbsentees = async () => {
    ...
    // FIX: Timezone agnostic day detection
    ...
    const absentees = users.filter(u => {
      ...
      // Absent if no record OR marked as medical
    ...
    absentees.forEach(teacher => {
      const duties = timetable.filter(t => {
        // Only look for base timetable duties (where date is null)
    ...
    setStatus({ type: 'success', message: `Scan successful: ${newRegistries.length} proxy slots identified for ${absentees.length} absentees.` });
    setIsProcessing(false);
  };
  ...
  const handleSmartProxy = async () => {
    ...
    try {
      ...
      const prompt = `Task: Academic Proxy Optimization for IHIS School.
      Date: ${selectedDate}, Section: ${activeSection}.
      
      PENDING DUTIES:
      ${pendings.map(p => `Slot P${p.slotId}, Class ${p.className}, Sub: ${p.subject}`).join('\n')}
      
      AVAILABLE STAFF:
      ${availableStaff.map(s => `${s.name} (ID: ${s.id}, Weekly Load: ${s.load})`).join('\n')}
      
      Rules:
      1. Prioritize teachers with lower weekly loads.
      2. Ensure a teacher is NOT busy in the specific slot (assume they are free if present and not assigned elsewhere).
      3. Return ONLY a JSON array of mappings: [{"subId": "...", "teacherId": "...", "teacherName": "..."}]`;
    ...
    <div className="space-y-6 animate-in fade-in duration-700 w-full px-2 pb-24">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 no-print">
        ...
        <div className="flex flex-wrap gap-2">
          {isManagement && (
            <>
              <button onClick={handleSmartProxy} disabled={isSmartProxyLoading} className="bg-gradient-to-r from-amber-400 to-amber-500 text-[#001f3f] px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase shadow-xl hover:scale-105 transition-all border border-white/20 flex items-center gap-2">
              ...
              <button onClick={() => setIsNewEntryModalOpen(true)} className="bg-white dark:bg-slate-800 text-[#001f3f] dark:text-white px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase shadow-md border border-slate-200 dark:border-slate-700 transition-all hover:bg-slate-50 active:scale-95">Log Manual</button>
              ...
              <button onClick={handleArchiveMatrix} className="bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300 px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase transition-all hover:bg-rose-50 hover:text-rose-500 active:scale-95">Archive Matrix</button>
            </>
          )}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 overflow-hidden flex flex-col min-h-[500px]">
        <div className="p-6 md:p-8 border-b border-gray-100 dark:border-slate-800 flex flex-col lg:flex-row items-center justify-between no-print bg-slate-50/50 gap-6">
           <div className="flex bg-white dark:bg-slate-950 p-1 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-x-auto scrollbar-hide w-full lg:w-auto">
              {(['PRIMARY', 'SECONDARY_BOYS', 'SECONDARY_GIRLS', 'SENIOR_SECONDARY_BOYS', 'SENIOR_SECONDARY_GIRLS'] as SectionType[]).map(s => (
                <button key={s} onClick={() => setActiveSection(s)} className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase transition-all whitespace-nowrap ${activeSection === s ? 'bg-[#001f3f] text-[#d4af37]' : 'text-slate-400'}`}>
                  {s.replace(/_/g, ' ')}
                </button>
              ))}
           </div>
        ...
        <div className="overflow-x-auto flex-1">
           {status && (
             <div className={`m-8 p-4 rounded-2xl text-[10px] font-black uppercase border animate-in slide-in-from-top ${
           ...
           <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[2.5rem] p-10 shadow-2xl space-y-8 animate-in zoom-in duration-300">
             ...
             <div className="space-y-4">
                <div className="space-y-1">
                   ...
                   <select value={newEntry.absentTeacherId} onChange={e => setNewEntry({...newEntry, absentTeacherId: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl px-4 py-3 text-xs font-bold dark:text-white">
                ...
                <div className="space-y-1">
                   ...
                   <select value={newEntry.className} onChange={e => setNewEntry({...newEntry, className: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl px-4 py-3 text-xs font-bold dark:text-white">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <select value={newEntry.slotId} onChange={e => setNewEntry({...newEntry, slotId: parseInt(e.target.value)})} className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl px-4 py-3 text-xs font-bold dark:text-white outline-none">
                  ...
                  <div className="space-y-1">
                    ...
                    <select value={newEntry.subject} onChange={e => setNewEntry({...newEntry, subject: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl px-4 py-3 text-xs font-bold dark:text-white">
             ...
             <button onClick={() => setIsNewEntryModalOpen(false)} className="w-full text-slate-400 font-black text-[11px] uppercase tracking-widest">Discard</button>
           </div>
        </div>
      )}
                         ...
                         <tr key={teacher.id} className="transition-all hover:bg-slate-50/50">
                            ...
                            <td className="px-8 py-6 text-center">
                                 {!available ? (
                                   ...
                                   <span className="text-xs font-black text-slate-500">{load.total}/35</span>
                                 )}
                            </td>
                            <td className="px-8 py-6 text-right">
                                 <button 
                                   ...
                                   className={`text-[9px] font-black uppercase px-5 py-2.5 rounded-xl shadow-md transition-all ${!available ? 'bg-slate-100 text-slate-300' : 'bg-[#001f3f] text-[#d4af37] hover:scale-105 active:scale-95'}`}
                                 >
