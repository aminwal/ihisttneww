...
import { User, SchoolConfig, TeacherAssignment, TimeTableEntry, LessonPlan, AppTab, Worksheet, SavedPlanRecord } from '../types.ts';
...
const LessonArchitectView: React.FC<LessonArchitectViewProps> = ({ 
  ...
  const [viewMode, setViewMode] = useState<'ARCHITECT' | 'VAULT'>('ARCHITECT');
  const [vaultPlans, setVaultPlans] = useState<SavedPlanRecord[]>([]);
  ...
  const fetchVaultPlans = async () => {
    if (!IS_CLOUD_ENABLED) {
       setError("Cloud Sync is disabled. Cannot access vault.");
       return;
    }
    if (isSandbox) {
       setVaultPlans([]);
    ...
    try {
      ...
      setVaultPlans(data as SavedPlanRecord[]);
    } catch (err: any) {
  ...
  const handleDeleteFromVault = async (id: string) => {
    if (!confirm("Remove this lesson plan from the vault?")) return;
    try {
      ...
      setError("Failed to delete from vault: " + err.message);
    }
  };

  const loadPlanFromVault = (record: SavedPlanRecord) => {
    setLessonPlan(record.plan_data);
    setTopic(record.topic);
    setSubject(record.subject);
    setGradeId(record.grade_id);
    if (record.section_id) setSectionId(record.section_id);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 pb-32 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-end gap-6 no-print px-2">
        ...
        <div className="flex bg-white dark:bg-slate-900 p-1 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
          <button 
            ...
            className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase transition-all ${viewMode === 'ARCHITECT' ? 'bg-[#001f3f] text-[#d4af37] shadow-md' : 'text-slate-400 hover:text-[#001f3f] dark:hover:text-white'}`}
          >
            Architect Studio
          </button>
          <button 
            onClick={() => { setViewMode('VAULT'); fetchVaultPlans(); }} 
            className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase transition-all flex items-center gap-2 ${viewMode === 'VAULT' ? 'bg-[#001f3f] text-[#d4af37] shadow-md' : 'text-slate-400 hover:text-[#001f3f] dark:hover:text-white'}`}
          >
            Faculty Vault
          </button>
        </div>
      ...
      {viewMode === 'VAULT' ? (
        <div className="bg-white dark:bg-slate-900 rounded-[3rem] p-8 md:p-12 shadow-2xl border border-slate-100 dark:border-slate-800 min-h-[600px] animate-in slide-in-from-bottom-4 duration-500">
          <div className="flex justify-between items-center mb-10">
            <div>
              <h2 className="text-2xl md:text-3xl font-black text-[#001f3f] dark:text-white uppercase italic tracking-tighter">My Faculty Vault</h2>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Securely stored lesson plans</p>
            </div>
            <button onClick={fetchVaultPlans} className="p-3 bg-slate-50 dark:bg-slate-800 text-slate-400 hover:text-amber-500 rounded-xl transition-colors">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
            </button>
          </div>
          
          {error && (
             <div className="p-4 mb-8 bg-rose-500/10 border border-rose-500/20 rounded-2xl">
               <p className="text-[10px] text-rose-400 font-bold text-center uppercase leading-relaxed">{error}</p>
             </div>
          )}

          {isLoadingVault ? (
            <div className="py-20 text-center text-slate-400 uppercase tracking-[0.2em] text-[10px] font-black flex flex-col items-center gap-4 opacity-50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {vaultPlans.map(plan => (
                <div key={plan.id} className="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-slate-200 dark:border-slate-700 flex flex-col group hover:border-amber-400 transition-all shadow-sm">
                   <div className="flex justify-between items-start mb-4">
                      <div className="pr-4">
                         <h3 className="text-sm font-black text-[#001f3f] dark:text-white uppercase leading-tight line-clamp-2">{plan.topic || plan.plan_data?.title || 'Untitled Lesson'}</h3>
                         <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1.5">{plan.subject}</p>
                      </div>
                      <div className="px-2 py-1 bg-white dark:bg-slate-900 rounded-lg text-[8px] font-black text-emerald-500 uppercase border border-slate-100 dark:border-slate-700 shrink-0">Saved</div>
                   </div>
                   
                   <div className="text-[10px] font-black text-slate-500 uppercase mt-auto pt-4 border-t border-slate-200 dark:border-slate-600 flex justify-between items-center">
                      <span>Grade: {config.grades.find(g => g.id === plan.grade_id)?.name || plan.grade_id}</span>
                      <span className="text-[8px] text-slate-400">{plan.date}</span>
                   </div>
                   
                   <div className="flex gap-2 mt-4">
                      <button onClick={() => loadPlanFromVault(plan)} className="flex-1 py-3 bg-[#001f3f] text-[#d4af37] rounded-xl text-[9px] font-black uppercase hover:bg-amber-400 hover:text-[#001f3f] transition-all shadow-md">Open Plan</button>
                      <button onClick={() => handleDeleteFromVault(plan.id!)} className="p-3 bg-white dark:bg-slate-800 text-rose-400 rounded-xl hover:bg-rose-500 hover:text-white border border-slate-200 dark:border-slate-600 transition-all shadow-sm">
            ...
            <div className="py-32 text-center text-slate-400 flex flex-col items-center gap-4 opacity-50">
               ...
               <p className="uppercase tracking-[0.3em] text-[11px] font-black">Vault is Empty</p>
            </div>
          )}
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 px-2 animate-in slide-in-from-bottom-4 duration-500">
