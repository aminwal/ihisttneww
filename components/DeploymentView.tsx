...
CREATE TABLE IF NOT EXISTS lesson_plans (
  ...
  const maskedUrl = IS_CLOUD_ENABLED ? 
    (localStorage.getItem('IHIS_CFG_VITE_SUPABASE_URL') || 'CLOUD_INJECTED_ENV').substring(0, 20) + "..." : 
    "NOT_CONFIGURED";
    ...
    <div className="space-y-8 animate-in fade-in duration-700 max-w-5xl mx-auto pb-24 px-4">
      <div className="bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-8">
        <div className="flex items-center gap-6">
          <div className={`w-20 h-20 rounded-3xl flex items-center justify-center border-4 ${dbStatus === 'connected' ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : dbStatus === 'local' ? 'bg-amber-50 border-amber-100 text-amber-600' : 'bg-rose-50 border-rose-100 text-rose-600'}`}>
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-3">
              Environment: {dbStatus === 'connected' ? 'Cloud Production' : dbStatus === 'local' ? 'Local Sandbox' : 'Disrupted Connection'}
            </p>
          </div>
        </div>
        <div className="flex gap-4">
           <div className={`px-5 py-2.5 rounded-xl border flex items-center gap-2 ${IS_CLOUD_ENABLED ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : 'bg-rose-50 border-rose-100 text-rose-600'}`}>
              <div className={`w-2 h-2 rounded-full ${IS_CLOUD_ENABLED ? 'bg-emerald-500' : 'bg-rose-500 animate-pulse'}`}></div>
              <span className="text-[10px] font-black uppercase">{IS_CLOUD_ENABLED ? 'Cloud Online' : 'Local Mode'}</span>
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 shadow-xl border border-slate-100 dark:border-slate-800 space-y-6">
           ...
           <div className="space-y-4">
              <div className="flex justify-between items-center p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl">
                 <div>
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Matrix Endpoint</p>
                 ...
                 <div className={`px-3 py-1 rounded-lg text-[7px] font-black uppercase ${IS_CLOUD_ENABLED ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                    {IS_CLOUD_ENABLED ? 'Linked' : 'Offline'}
                 </div>
              </div>

              <div className="flex justify-between items-center p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl">
                 <div>
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">AI Logic Pulse</p>
                    <p className="text-[10px] font-bold text-[#001f3f] dark:text-white uppercase italic">
                       {matrixPulse === 'PULSING' ? 'Pinging Logic...' : matrixPulse === 'ONLINE' ? 'Bridge Active' : matrixPulse === 'OFFLINE' ? 'Deployment Required' : 'Awaiting Test'}
                    </p>
                 </div>
                 <button 
                 ...
                 >
                    {matrixPulse === 'PULSING' ? '...' : 'Pulse Check'}
                 </button>
              </div>
           </div>

           {matrixPulse === 'OFFLINE' && (
              <div className="p-5 bg-rose-50 dark:bg-rose-900/10 border border-rose-100 dark:border-rose-900 rounded-[1.5rem] space-y-3">
                 <p className="text-[9px] font-black text-rose-600 uppercase tracking-widest flex items-center gap-2">
                    ...
                    Action Required in GitHub Terminal
                 </p>
                 <p className="text-[10px] text-rose-500 font-medium italic leading-relaxed">
                    The database is linked, but your Edge Function code exists only in GitHub. You must push it to the cloud using the <b>Deployment Step 2</b> command below.
                 </p>
              </div>
           )}
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 shadow-xl border border-slate-100 dark:border-slate-800 space-y-6">
           <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-xl font-black text-[#001f3f] dark:text-white uppercase italic tracking-tighter">GitHub Build Audit</h2>
           </div>
      ...
      <div className="bg-[#001f3f] rounded-[3rem] p-10 shadow-2xl border border-white/10 space-y-8">
         <div className="flex items-center gap-4">
            ...
            <h2 className="text-2xl font-black text-white uppercase italic tracking-tighter">GitHub / Codespace Integration Protocol</h2>
         </div>
         
         <div className="space-y-6">
            <div className="space-y-2">
               <p className="text-[9px] font-black text-white/40 uppercase tracking-widest ml-4">Step 1: Link Cloud Project in GitHub Terminal</p>
            ...
            <div className="space-y-2">
               <p className="text-[9px] font-black text-white/40 uppercase tracking-widest ml-4">Step 2: Push Edge Logic from GitHub to Cloud</p>
               <div className="bg-slate-950 p-6 rounded-2xl border border-white/5 font-mono text-[11px] text-emerald-400 flex justify-between items-center group">
               ...
               <p className="text-[8px] text-amber-400/50 italic ml-4">This resolves the GATING_ERROR by making the AI code live.</p>
            </div>
            <div className="space-y-2">
               <p className="text-[9px] font-black text-white/40 uppercase tracking-widest ml-4">Step 3: Authorize Matrix API Key (Run in GitHub Terminal)</p>
               <div className="bg-slate-950 p-6 rounded-2xl border border-white/5 font-mono text-[11px] text-emerald-400 flex justify-between items-center group">
      ...
      <div className="text-center pb-12">
        <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Developed by Ahmed Minwal • Operational Architecture V8.4</p>
      </div>
    </div>
  );
};
