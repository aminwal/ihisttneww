...
const DeploymentView: React.FC = () => {
  ...
  const handleManualSave = () => {
    if (!urlInput.trim() || !keyInput.trim()) { 
      setSaveStatus("Error: Both URL and Key are required for a manual override."); 
      return; 
    }
    ...
    setSaveStatus("Success: Credentials Secured. Synchronizing Cluster...");
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-3">
              Status: {dbStatus === 'connected' ? 'Cloud Integrated' : dbStatus === 'local' ? 'Local Standalone' : 'Handshake Error'}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <section className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-xl border border-slate-100 p-8 space-y-6">
            <div className="flex justify-between items-center">
               <h2 className="text-xl font-black uppercase italic text-[#d4af37]">Manual Override</h2>
               {localStorage.getItem('IHIS_CFG_VITE_SUPABASE_URL') && (
                 <button onClick={handleClearOverride} className="text-[8px] font-black text-rose-500 uppercase border-b border-rose-500">Clear Overrides</button>
               )}
            </div>
            <div className="space-y-4">
              ...
              <button onClick={handleManualSave} className="w-full bg-[#001f3f] text-[#d4af37] py-6 rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-xl">Activate Manual Link</button>
            </div>
          ...
          <section className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-xl border border-slate-100 p-8 flex flex-col dark:border-slate-800">
             ...
             <p className="mt-4 text-[9px] font-medium text-slate-400 italic">Run this script in your Supabase SQL Editor to prepare the database for synchronization.</p>
          </section>
      </div>
