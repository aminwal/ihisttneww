...
const ProfileView: React.FC<ProfileViewProps> = ({ user, setUsers, setCurrentUser, config }) => {
  ...
  const handleSyncTelegram = () => {
    ...
    // 1. Open Telegram Bot with /start <userId>
    const botUrl = `https://t.me/${config.telegramBotUsername}?start=${user.id}`;
    window.open(botUrl, '_blank');

    // 2. Start Polling for Discovery
    let attempts = 0;
    const MAX_ATTEMPTS = 40; // 2 minutes approx (3s intervals)
  ...
  const saveTelegramId = async (chatId: string) => {
    try {
      ...
      stopSyncPolling("Failed to save credentials.");
    }
  };
  ...
  return (
    <div className="max-w-xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-32 px-2">
      ...
      {/* Modern Automatic Telegram Synchronization Card */}
         ...
         <div className="relative z-10 space-y-5">
            ...
            {user.telegram_chat_id ? (
              <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-6 flex items-center justify-between">
                <div>
                   <p className="text-[10px] font-black text-emerald-400 uppercase tracking-widest">Status: Matrix Active</p>
                   <p className="text-[8px] text-white/40 mt-1 uppercase font-bold">Encrypted ID: {user.telegram_chat_id.substring(0,4)}****</p>
                </div>
              ...
              <div className="space-y-4">
                 <p className="text-xs font-medium text-white/70 leading-relaxed italic">
                   Link your Telegram account to receive instant notifications when you are assigned a proxy duty.
                 </p>
                 <button 
                   onClick={handleSyncTelegram}
                   disabled={isSyncingTelegram}
                   className={`w-full flex items-center justify-center gap-3 py-5 rounded-2xl font-black text-xs uppercase tracking-widest transition-all shadow-xl active:scale-95 ${
                     isSyncingTelegram 
                     ? 'bg-amber-400 text-[#001f3f] animate-pulse' 
                 ...
                 {isSyncingTelegram && (
                   <p className="text-[9px] text-amber-400 font-bold text-center uppercase tracking-widest animate-bounce">
                     Hit "START" in the Telegram chat to complete sync
                   </p>
                 )}
              </div>
            )}
        ...
        {biometricSupported && (
          <div className="mb-10 p-6 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-slate-100 dark:border-slate-800">
            <div className="flex items-center justify-between">
              ...
              <button onClick={handleBiometricToggle} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${biometricEnrolled ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-700'}`}>
        ...
        <form onSubmit={handleUpdate} className="space-y-6">
          <div className="grid grid-cols-1 gap-6">
            <div className="space-y-2">
              ...
              <input type="text" value={user.employeeId} readOnly className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-2 border-transparent rounded-2xl font-black text-sm text-slate-400 cursor-not-allowed outline-none italic" />
            </div>
            <div className="space-y-2">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">WhatsApp Liaison</label>
              <input type="text" placeholder="97333000000" value={phoneNumber} onChange={e => setPhoneNumber(e.target.value)} className="w-full px-6 py-4 bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 focus:border-emerald-400 rounded-2xl font-bold text-sm dark:text-white outline-none transition-all placeholder:text-slate-300 shadow-sm" />
            </div>
            <div className="space-y-2">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Registry Password</label>
              <input type="text" value={password} onChange={e => setPassword(e.target.value)} required className="w-full px-6 py-4 bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 focus:border-amber-400 rounded-2xl font-bold text-sm dark:text-white outline-none transition-all shadow-sm" />
            </div>
          </div>
          ...
          <button type="submit" disabled={loading} className="w-full bg-[#001f3f] text-[#d4af37] py-5 rounded-2xl font-black text-xs uppercase tracking-[0.3em] shadow-2xl hover:bg-slate-900 active:scale-95 transition-all disabled:opacity-50">
      ...
      <div className="text-center opacity-30">
        <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.4em]">IHIS Gateway • Build 2.7</p>
      </div>
    </div>
  );
};
