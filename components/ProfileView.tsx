...
const ProfileView: React.FC<ProfileViewProps> = ({ user, setUsers, setCurrentUser, config }) => {
  ...
  const saveTelegramId = async (chatId: string) => {
    try {
      if (IS_CLOUD_ENABLED) await supabase.from('profiles').update({ telegram_chat_id: chatId }).eq('id', user.id);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      if (IS_CLOUD_ENABLED) {
        const { error } = await supabase.from('profiles').update({ email, password, phone_number: phoneNumber || null }).eq('id', user.id);
  ...
  return (
    <div className="max-w-xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-32 px-2">
      ...
      <div className="bg-[#001f3f] rounded-[2.5rem] p-8 shadow-2xl border border-white/10 space-y-6 relative overflow-hidden group">
         ...
         <div className="relative z-10 space-y-5">
            <div className="space-y-1">
               ...
               <p className="text-[9px] font-medium text-white/50 uppercase tracking-widest leading-relaxed">Instant Proxy Duty Discovery System</p>
            </div>
            {user.telegram_chat_id ? (
              ...
              <div className="space-y-4">
                 <p className="text-xs font-medium text-white/70 leading-relaxed italic">Link your Telegram account to receive instant notifications when you are assigned a proxy duty.</p>
                 <button onClick={handleSyncTelegram} disabled={isSyncingTelegram} className={`w-full flex items-center justify-center gap-3 py-5 rounded-2xl font-black text-xs uppercase tracking-widest transition-all shadow-xl active:scale-95 ${isSyncingTelegram ? 'bg-amber-400 text-[#001f3f] animate-pulse' : 'bg-[#0088cc] text-white hover:bg-[#0077b5]'}`}>{isSyncingTelegram ? (<><div className="w-4 h-4 border-2 border-[#001f3f] border-t-transparent rounded-full animate-spin"></div>Polling Matrix...</>) : (<><svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.38-.94-2.23-1.5-.99-.65-.35-1.01.22-1.59.15-.15 2.71-2.48 2.76-2.69.01-.03.01-.14-.07-.2-.08-.06-.19-.04-.27-.02-.12.02-1.96 1.25-5.54 3.69-.52.36-1 .53-1.42.52-.47-.01-1.37-.26-2.03-.48-.82-.27-1.47-.42-1.42-.88.03-.24.35-.49.96-.75 3.78-1.65 6.31-2.73 7.57-3.24 3.59-1.47 4.34-1.73 4.82-1.73.11 0 .35.03.5.15.13.09.16.22.18.31.02.08.02.24.01.41z"/></svg>Sync Telegram Account</>)}</button>
                 {isSyncingTelegram && (<p className="text-[9px] text-amber-400 font-bold text-center uppercase tracking-widest animate-bounce">Hit "START" in the Telegram chat to complete sync</p>)}
              </div>
            )}
         </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 md:p-12 shadow-2xl border border-slate-100 dark:border-slate-800">
        ...
        {biometricSupported && (<div className="mb-10 p-6 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-slate-100 dark:border-slate-800"><div className="flex items-center justify-between"><div className="flex items-center gap-4"><div className={`w-10 h-10 rounded-2xl flex items-center justify-center ${biometricEnrolled ? 'bg-emerald-500 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-400'}`}><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 11c0 3.517-1.009 6.799-2.753 9.571m-3.44-2.04l.054-.09m1.916-5.111a10.273 10.273 0 01-1.071 4.76m16.125-9.286a20.587 20.587 0 01-1.184 8.023m-1.258 2.527c-.887 1.413-1.952 2.68-3.152 3.752m-2.456 2.108a16.033 16.033 0 01-5.995-1.1m7.532-5.664a10.513 10.513 0 01-3.136 3.553m-.73-3.135c.342.333.667.697.973 1.088m3.963-6.176a12.42 12.42 0 01-.338 4.466M9 21v-3.338c0-.58-.306-1.118-.812-1.41a10.737 10.737 0 01-3.207-2.542m14.056-6.41A9.147 9.147 0 0017.307 3M15 3.568A10.098 10.098 0 0118 10c0 .329-.016.655-.047.976m-3.805 3.69A8.147 8.147 0 0112 15m-5.333-3.945c.07-.468.145-.932.227-1.396M14 3a2 2 0 114 0c0 .553-.447 1-1 1h-1V3z" /></svg></div><div><h3 className="text-xs font-black text-[#001f3f] dark:text-white uppercase tracking-tight">Biometric Gateway</h3><p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{biometricEnrolled ? 'Active' : 'Deactivated'}</p></div></div><button onClick={handleBiometricToggle} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${biometricEnrolled ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-700'}`}><span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${biometricEnrolled ? 'translate-x-6' : 'translate-x-1'}`} /></button></div></div>)}
        <form onSubmit={handleUpdate} className="space-y-6"><div className="grid grid-cols-1 gap-6"><div className="space-y-2"><label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Staff Code</label><input type="text" value={user.employeeId} readOnly className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-2 border-transparent rounded-2xl font-black text-sm text-slate-400 cursor-not-allowed outline-none italic" /></div><div className="space-y-2"><label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">WhatsApp Liaison</label><input type="text" placeholder="97333000000" value={phoneNumber} onChange={e => setPhoneNumber(e.target.value)} className="w-full px-6 py-4 bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 focus:border-emerald-400 rounded-2xl font-bold text-sm dark:text-white outline-none transition-all placeholder:text-slate-300 shadow-sm" /></div><div className="space-y-2"><label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Registry Password</label><input type="text" value={password} onChange={e => setPassword(e.target.value)} required className="w-full px-6 py-4 bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 focus:border-amber-400 rounded-2xl font-bold text-sm dark:text-white outline-none transition-all shadow-sm" /></div></div>{status && (<div className={`px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all animate-in zoom-in duration-300 ${status.type === 'success' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : status.type === 'info' ? 'bg-sky-50 text-sky-600 border-sky-100' : status.type === 'warning' ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-red-50 text-red-600 border-red-100'}`}>{status.message}</div>)}<button type="submit" disabled={loading} className="w-full bg-[#001f3f] text-[#d4af37] py-5 rounded-2xl font-black text-xs uppercase tracking-[0.3em] shadow-2xl hover:bg-slate-900 active:scale-95 transition-all disabled:opacity-50">{loading ? 'SYNCING...' : 'AUTHORIZE UPDATES'}</button></form>
      </div>

      <div className="text-center opacity-30"><p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.4em]">IHIS Gateway • Build 3.0</p></div>
    </div>
  );
};
