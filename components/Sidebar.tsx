...
const Sidebar: React.FC<SidebarProps> = ({ role, activeTab, setActiveTab, config }) => {
  ...
  const navItems: { id: AppTab; label: string; icon: string }[] = [
    ...
    ...(isManagement ? [{ id: 'reports' as AppTab, label: 'Institutional Analytics', icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2' }] : []),
  ...
  return (
    <aside className="w-16 md:w-64 bg-[#00112b]/95 backdrop-blur-md text-white flex flex-col transition-all duration-300 border-r border-white/10 shadow-2xl z-20 overflow-hidden shrink-0 hover:scale-[1.01] origin-left">
      <button 
        onClick={() => setActiveTab('dashboard')}
        className="p-4 md:p-6 flex items-center space-x-3 mb-6 md:mb-10 border-b border-white/5 justify-center md:justify-start hover:bg-white/5 transition-colors group text-left outline-none"
      >
        <div className="w-10 h-8 md:w-12 md:h-10 bg-[#d4af37] rounded-lg md:rounded-xl flex items-center justify-center font-black text-xs md:text-base text-[#001f3f] shadow-[0_0_15px_rgba(212,175,55,0.3)] shrink-0 group-hover:scale-110 group-hover:-rotate-6 group-hover:shadow-[0_0_25px_rgba(212,175,55,0.5)] transition-all duration-300 ease-out">
          IHIS
        </div>
        <div className="hidden md:block">
          ...
          <span className="block text-[8px] font-black text-amber-200 uppercase tracking-[0.3em] opacity-70">Portal</span>
        </div>
      </button>
      
      <nav className="flex-1 space-y-2 px-2 md:px-3 overflow-y-auto scrollbar-hide">
        {navItems.map(item => (
          <button
            key={item.id}
            onClick={() => setActiveTab(item.id)}
            title={item.label}
            className={`w-full flex items-center justify-center md:justify-start md:space-x-3 p-3 rounded-xl transition-all duration-300 ${
              activeTab === item.id 
          ...
          >
            ...
            <span className="hidden md:inline font-bold tracking-tight text-sm truncate">{item.label}</span>
          </button>
        ))}
      </nav>
      
      <div className="p-4 md:p-6 text-[8px] md:text-[9px] font-black text-amber-200/30 uppercase tracking-widest hidden md:block">
