...
const BatchTimetableView: React.FC<BatchTimetableViewProps> = ({ 
    ...
    if (el.type === 'IMAGE') {
      return (
        <div key={el.id} style={{ 
          ...
          <img src={el.content} crossOrigin="anonymous" style={{ width: `${el.style.width}px`, height: `${el.style.height}px`, objectFit: 'contain', opacity: el.style.opacity ?? 1, filter: el.style.grayscale ? 'grayscale(100%)' : 'none' }} alt="Brick" />
        </div>
      );
    }
    return (
  ...
  const renderSingleTimetable = (entity: { id: string, name: string, type: string }) => {
    ...
    const pageSizeMap: Record<string, {w: number, h: number}> = {
      'a4': {w: 297, h: 210}, 'a3': {w: 420, h: 297}, 'letter': {w: 279.4, h: 215.9}, 'legal': {w: 355.6, h: 215.9}
    };
                    ...
                    {slots.map(s => {
                       ...
                       return (
                         <td key={s.id} style={{ border: `1px solid ${template.tableStyles.borderColor}`, padding: `${template.tableStyles.cellPadding}px`, textAlign: 'center' }}>
                            {entries.map(e => {
                               // REFINED RENDERING: For Class (isC), show block heading. For Staff (isS), show subject.
                               const displayLabel = (isC && e.blockId) ? e.blockName : e.subject;
                               return (
                                 <div key={e.id} className="space-y-0.5">
                                    <p style={{ fontSize: `${template.tableStyles.fontSize}px` }} className={`font-black uppercase ${e.blockId && template.visibility.showBlockIdentity ? 'text-amber-600' : 'text-[#001f3f]'}`}>{displayLabel}</p>
               ...
               {sects.map((sec, sIdx) => (
                 <tr key={sec.id} style={{ height: `${template.tableStyles.rowHeight}mm`, backgroundColor: template.tableStyles.stripeRows && sIdx % 2 !== 0 ? '#f8fafc' : 'transparent' }}>
                    ...
                    {wSlots.map(s => {
                       ...
                       // REFINED RENDERING: Master matrix shows group Heading.
                       const displayLabel = (e?.blockId) ? e.blockName : e?.subject;

                       return (
                         <td key={s.id} style={{ border: `1px solid ${template.tableStyles.borderColor}`, padding: `${template.tableStyles.cellPadding}px`, textAlign: 'center' }}>
                            {e ? (
                              <div className="space-y-1">
                                 <p style={{ fontSize: '14px' }} className={`font-black uppercase ${e.blockId && template.visibility.showBlockIdentity ? 'text-amber-600' : 'text-[#001f3f]'}`}>{displayLabel}</p>
